...
def build_fake_data(reference_data_path: str, size: int = 100) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    fake_data = pd.DataFrame(fake_data, columns=NUMERICAL_FEATURES + CATEGORICAL_FEATURES)

    fake_targets = pd.DataFrame(data[TARGET].sample(size, replace=True)).reset_index(drop=True)
